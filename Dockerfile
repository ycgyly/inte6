...
RUN mkdir  $HOME/.vnc
RUN echo 'yly' | vncpasswd -f > $HOME/.vnc/passwd
RUN echo '/bin/env  MOZ_FAKE_NO_SANDBOX=1  dbus-launch xfce4-session'  > $HOME/.vnc/xstartup
RUN chod 600 $HOME/.vnc/passwd
RUN chmod 755 $HOME/.vnc/xstartup
RUN echo 'whoami ' >>/yly.sh
RUN echo 'cd ' >>/yly.sh
RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/yly.sh
RUN echo 'cd /noVNC-1.2.0' >>/yly.sh
RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/yly.sh
RUN chmod 755 /yly.sh
EXPOSE 8900
CMD  /yly.sh
